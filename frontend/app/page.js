  ...
  async function updateUser(id, user) {
    ...
    setUserRows((current) => {
      return current.map((row) => {
        if (userActionId(row) === id) {
          const nextRow = [...row];
          nextRow[1] = user.name;
          nextRow[2] = user.role;
          nextRow[3] = user.unit || "-";
          nextRow[4] = user.status;
          nextRow[5] = user.email || "";
          nextRow[7] = user.jabatan || "";
          return nextRow;
        }
        return row;
      });
    });
    setConfirm({ title: "User dummy diperbarui", body: `Data ${user.name} berhasil diperbarui secara lokal.` });
    return true;
  }

  async function handleToggleUserStatus(id, name, currentStatus) {
    if (!isDummyToken()) {
      try {
        await apiFetch(`/users/${id}/toggle-status`, {
          method: "PATCH"
        });
        await loadUsers();
        setConfirm({ title: "Status diubah", body: `Status ${name} berhasil diubah.` });
      } catch (error) {
        setConfirm({ title: "Gagal mengubah status", body: error.message });
      }
      return;
    }
    ...
    setUserRows((current) => {
      return current.map((row) => {
        if (userActionId(row) === id) {
          const nextRow = [...row];
          nextRow[4] = currentStatus === "Aktif" ? "Nonaktif" : "Aktif";
          return nextRow;
        }
        return row;
      });
    });
    setConfirm({ title: "Status dummy diubah", body: `Status ${name} berhasil diubah secara lokal.` });
  }
...
function AdminUserManagement({
  ...
  const filteredRows = useMemo(() => {
    ...
    return displayRows.filter(row => {
      const searchVal = (userParams?.search || "").toLowerCase();
      const matchesSearch = !searchVal || 
        String(row[1] || "").toLowerCase().includes(searchVal) ||
        String(row[5] || "").toLowerCase().includes(searchVal) ||
        String(row[6]?.username || "").toLowerCase().includes(searchVal);
      
      const roleVal = userParams?.role || "";
      const matchesRole = !roleVal || row[2] === roleVal;

      const statusVal = userParams?.status || "";
      const matchesStatus = !statusVal || String(row[4]).toLowerCase() === statusVal;

      return matchesSearch && matchesRole && matchesStatus;
    });
  }, [displayRows, userParams, isDummyMode]);

  const totalPages = isDummyMode ? Math.max(1, Math.ceil(filteredRows.length / pageSize)) : (userMeta?.totalPages || 1);
  const totalCountVal = isDummyMode ? filteredRows.length : (userMeta?.totalCount || 0);
  const currentPage = isDummyMode ? page : (userParams?.page || 1);

  const visibleRows = useMemo(() => {
    if (isDummyMode) {
      return filteredRows.slice((page - 1) * pageSize, page * pageSize);
    }
    return rows; // Backend already returned limited rows!
  }, [rows, filteredRows, page, isDummyMode]);

  const start = isDummyMode ? (filteredRows.length ? (page - 1) * pageSize + 1 : 0) : (rows.length ? (currentPage - 1) * 10 + 1 : 0);
  const end = isDummyMode ? Math.min(page * pageSize, filteredRows.length) : Math.min(currentPage * 10, totalCountVal);
  ...
  useEffect(() => {
    if (isDummyMode) {
      ...
      setUserParams(prev => ({ ...prev, page: 1 }));
    }
  }, [userParams?.search, userParams?.role, userParams?.status, isDummyMode]);

  const handlePrevPage = () => {
    if (isDummyMode) {
      setPage(prev => Math.max(1, prev - 1));
    } else {
      setUserParams(prev => ({ ...prev, page: Math.max(1, prev.page - 1) }));
    }
  };
  ...
  const handleNextPage = () => {
    if (isDummyMode) {
      ...
      setUserParams(prev => ({ ...prev, page: Math.min(totalPages, prev.page + 1) }));
    }
  };
  ...
  const handleGoToPage = (p) => {
    if (isDummyMode) {
      ...
      setUserParams(prev => ({ ...prev, page: p }));
    }
  };
  ...
  return (
    <section className="adminUsersPage">
      ...
      <article className="adminUsersPanel">
        <div className="adminUsersToolbar adminUsersToolbarActions" style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
          <input
            value={userParams?.search || ""}
            onChange={(e) => {
              const val = e.target.value;
              if (isDummyMode) {
                setUserParams(prev => ({ ...prev, search: val }));
              } else {
                setUserParams(prev => ({ ...prev, search: val, page: 1 }));
              }
            }}
          ...
          <select
            ...
            onChange={(e) => {
              const val = e.target.value;
              if (isDummyMode) {
                setUserParams(prev => ({ ...prev, role: val }));
              } else {
                setUserParams(prev => ({ ...prev, role: val, page: 1 }));
              }
            }}
            className="adminUsersFilter"
          ...
          <select
            ...
            onChange={(e) => {
              const val = e.target.value;
              if (isDummyMode) {
                setUserParams(prev => ({ ...prev, status: val }));
              } else {
                setUserParams(prev => ({ ...prev, status: val, page: 1 }));
              }
            }}
            className="adminUsersFilter"
            style={{ padding: "8px 12px", borderRadius: "6px", border: "1px solid #ddd", fontSize: "14px" }}
                  ...
                  <td>
                    <div className="adminUsersActions">
                      ...
                      <button
                        ...
                        className="toggle"
                        disabled={isProtectedAdminUser(row)}
                        aria-label={`Toggle status ${row[1]}`}
                        onClick={() => onUserToggleStatus?.(userActionId(row), row[1], row[4])}
                        style={{ color: row[4] === "Aktif" ? "#d94d54" : "#1c9d73" }}
                      >
                        <LineIcon name={row[4] === "Aktif" ? "user-minus" : "user-plus"} />
                      </button>
                      <button
                        type="button"
                        className="key"
        ...
        <footer className="adminUsersFooter">
          <span>Menampilkan {start} - {end} dari {totalCountVal} pengguna</span>
          <div className="adminUsersPager">
            <button type="button" disabled={currentPage <= 1} onClick={handlePrevPage}>&lt;</button>
            {Array.from({ length: totalPages }, (_, index) => index + 1).map((item) => (
              <button type="button" className={currentPage === item ? "active" : ""} key={item} onClick={() => handleGoToPage(item)}>{item}</button>
            ))}
            <button type="button" disabled={currentPage >= totalPages} onClick={handleNextPage}>&gt;</button>
            <select defaultValue="10"><option>10 / halaman</option></select>
          </div>
        </footer>
      </article>
    </section>
...
function ModuleView({
  ...
  const data = useMemo(() => {
    if (view === "Pengguna") return sourceRows;
    ...
    return (
      <AdminUserCreate
        ...
        onCreateUser={async (user) => {
          let saved;
          if (editUser) {
            saved = await onUserUpdate?.(editUser.id, user);
          } else {
            saved = await onCreateUser(user);
          }
          if (saved !== false) {
            setOpenForms((current) => ({ ...current, [view]: false }));
            setEditUser(null);
    ...
    <section className="adminUserCreatePage">
      <header className="dispositionCreateHeader">
        <div>
          ...
          <p>{isEdit ? "Ubah detail informasi akun pengguna dan atur unit kerja." : "Tambahkan akun pengguna baru dan atur role sesuai hak akses sistem."}</p>
        </div>
      </header>
          ...
          <article className="dispositionFormCard">
            ...
            <div className="adminUserFields">
              <label>Nama Lengkap <span>*</span>
                <input name="name" defaultValue={initialData?.name || ""} placeholder="Masukkan nama lengkap" required />
              </label>
              <label>Username <span>*</span>
                <input name="username" defaultValue={initialData?.username || ""} placeholder="Contoh: operator2" disabled={isEdit} required={!isEdit} />
              </label>
              <label>Email
